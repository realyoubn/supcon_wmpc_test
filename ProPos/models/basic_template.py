...
# 将 resize 提升为顶层可序列化对象，保持与原逻辑相同的行为
class ResizeTransform:
    ...
    def __call__(self, img):
        # 使用 torchvision.transforms.functional.resize 保持行为一致
        return TF.resize(img, self.size, interpolation=self.interpolation)
...
class TrainTask(object):
    ...
    def build_default_options():
        ...
        parser.add_argument('--data_folder', type=str, default='D:/2025/python/ProPos/datasets',
    ...
    @staticmethod
    def test_transform(dataset_name_or_normalize, size=32):
        """
        返回用于测试/验证的 transform，兼容两种调用：
        - 传入 dataset_name (str) -> 使用 TrainTask.normalize(dataset_name)
        - 传入已经构造好的 normalize transform 对象 -> 直接使用它
        """
        # 如果传入的是字符串，按 dataset_name 处理；否则认为传入的是 normalize transform
        if isinstance(dataset_name_or_normalize, str):
            dataset_name = dataset_name_or_normalize
            normalize = TrainTask.normalize(dataset_name)
        else:
            dataset_name = None
            normalize = dataset_name_or_normalize

        if dataset_name is not None and dataset_name.lower().startswith('imagenet'):
            transform_list = [
                ResizeTransform(256),
    ...
    def fit(self):
        ...
        self.psedo_labeling(n_iter)
        while True:
            # Some samplers (e.g. custom RandomSampler) may not implement set_epoch.
            # Only call set_epoch when the sampler provides that method (e.g. DistributedSampler).
